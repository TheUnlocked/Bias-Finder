...
function postVersionInfo(){
    browser.storage.sync.get("postedVersion", function(items){
        if (!("postedVersion" in items) || items["postedVersion"] != browser.runtime.getManifest().version){

            browser.storage.sync.set({"postedVersion": browser.app.getDetails().version});

            buffer.push({
                hitType: 'event',
                eventCategory: category,
                eventAction: 'versionInfo',
                eventLabel: browser.app.getDetails().version
            });
